...
def send_warning_email(key, user, sensor, readings):
    subject = f"[CelloBarn] {key.title()} Notification"
    sender = "noreply@cellobarn.com"
    to = [user.email]
    body = render_template(
        "email/warning.txt", key=key, sensor=sensor, readings=readings
    )
    html = render_template(
        "email/warning.html", key=key, sensor=sensor, readings=readings
    )
    send_email(subject, sender, to, text_body=body, html_body=html, sync=True)
    ...
    for sensor in sensors:
        ...
        for reading in readings:
            if reading.value < sensor.min_temp or reading.value > sensor.max_temp:
                send_warning_email(key=key, user=user, sensor=sensor, readings=readings)
                break
